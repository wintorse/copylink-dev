...
>
> [UserScript 版](https://github.com/wintorse/copylink-dev-user-js)をご利用ください。
>
> _開発者向け：_ ホスト権限を付与し、ショートカットを「グローバル」モードに設定すると動作させることは可能です。
>
> [v1.5.0](https://github.com/wintorse/copylink-dev/releases/tag/v1.5.0)ではホスト権限が付与されているため、インストールしショートカットを設定することで利用可能です。
>
...
加えて、Google スプレッドシートでは選択した範囲へのリンクをコピーすることができます。
...
1. このリポジトリをクローンする
2. `pnpm i` で開発依存関係をインストールする
3. `pnpm run build` を実行する
...
1. このリポジトリをクローンする
2. `pnpm i` を実行して依存関係をインストールする
3. `pnpm run build-firefox` を実行する
...
| スプレッドシートの選択範囲のリンクをコピー | `Ctrl+R`       | `Alt+R`       |

ショートカットは[設定](chrome://extensions/shortcuts)から変更できます。

Slack 絵文字名の変更・スプレッドシートの選択範囲のリンク形式の変更は拡張機能アイコンをクリックして行えます。
